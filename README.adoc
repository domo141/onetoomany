...
=== link:rdiff.pl[rdiff.pl]

Create a tunnel from one host to another, then on this another host,
execute `rdiff.pl file1 file1` and do the diffing on this first host.

The `rdiff.pl` must be available on both hosts; the same program is
...
....
   $ ./rdiff.pl . xxdiff ssh 127.0.0.1 $PWD/rdiff.pl
....

Then on another terminal
....
   $ git cat-file -p HEAD^:README.adoc | ./rdiff.pl - README.adoc
....
