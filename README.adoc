...
=== link:src/ldpreload-trace-execves.c[ldpreload-trace-execves.c]

Wraps execve(2) and various library functions that eventually call
execve(2). Prints some info of the call to stderr. Probably most
useful as a base for mangling the calls; otherwise strace(1) and
ltrace(1) (with the -e option) are usually more useful.
